...
| `tick_all.py` | The timer's entry point: runs `publish.py tick` for every registered client with Meta ids (registry lives outside the repo). Per-client failure isolation and per-client logs |
