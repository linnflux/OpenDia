...
- **Send pile** — every Gmail draft edited in the last 60 days, age-sorted (amber ≥3d, red ≥7d), each worth 4 board points. Recipients resolve to clients via alias emails, inbox-history sender mappings (domain-level only when unambiguous), and company websites; matched cards get a jump chip (strong subject match, else the client's supervisor or best-ranked open card as a ≈best-guess). Read-only by policy — nothing sends from the dashboard; send the draft in Gmail and the row clears itself (the day's seen-set in `sendpile.json` turns a vanished draft into earned points). Older fossils surface only as a count.
