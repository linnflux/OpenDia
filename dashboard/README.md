...
- **Day board** — every item on the view carries a value (THE FIRE 10, recs 5, attention items 3, inbox items/actions 2); the bar fills toward today's total as items clear. Clearing happens via the ✓ on rec/attention items (self-reported, `checks.json`, with undo), automatically when an item's card is completed today, or through the operator inbox's own ack/resolve. Cleared items move to a "Cleared today" strip. Yesterday's completion % is the notch to race (percent, not points — boards differ in size); `briefing/scores.json` keeps ~a month of daily high-water marks.
- **Send pile** — every row carries **✎ Mailroom**, opening the draft workspace: the Mailroom renders the REAL Gmail draft in an editable pane (Save to Gmail = `users.drafts.update`, preserving headers/threading; ↻ re-resolves by thread if the session recreated the draft under a new id) with the thread and card context below, and the standing session framed on that draft — so "check nothing new came up, then drop the last sentence" works in the composer. Draft *update* is the only Gmail write anywhere; nothing can send. Every Gmail draft edited in the last 60 days, age-sorted (amber ≥3d, red ≥7d), each worth 4 board points. Recipients resolve to clients via alias emails, inbox-history sender mappings (domain-level only when unambiguous), and company websites; matched cards get a jump chip (strong subject match, else the client's hub card or best-ranked open card as a ≈best-guess). Read-only by policy — nothing sends from the dashboard; send the draft in Gmail and the row clears itself (the day's seen-set in `sendpile.json` turns a vanished draft into earned points). Older fossils surface only as a count.
...
- **Hub check-ins** — roster = companies whose card carries the `hub` tag (the company-level coordinator card; "supervisor" is the ODA agent role, a different thing). Each morning's verdict is also appended as a ⌂-dated line to the hub card's notes (last 10 kept), so the hub's long-memory session inherits the picture when resumed. The server gathers each company's signals (open cards, live session gate states, open inbox items, recent client email) and a tool-less model call judges ok vs attention with specific items.
