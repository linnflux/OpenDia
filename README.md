...
- **Tiered by what it may touch.** Tier 1 is performed on approval (card and Notion updates, drafting a reply, opening a Room, read-only diagnostics). Tier 3 — servers, live client systems, calendar, money, git — is never performed; it becomes a handoff brief and a working session. Nothing sends email: a drafted reply lands in Gmail as a draft and a human sends it from there.
