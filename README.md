...
- **Tiered by what it may touch.** Two named tiers, not numbers. `"performed"` runs on approval (card and Notion updates, drafting a reply, opening a Room, read-only diagnostics). `"handoff"` — servers, live client systems, calendar, money, git — is never performed; it becomes a handoff brief and a working session. Anything unrecognised is read as `"handoff"`, so the safe default is the one that requires no guessing. Nothing sends email: a drafted reply lands in Gmail as a draft and a human sends it from there.
