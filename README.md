...
A lightweight dashboard whose core views — **Board** (project kanban), **Inbox** (email pipeline queue), and **Clients** (company-centric report view) — provide a visual interface to the SQLite database, with a nav sidebar that has since grown work views (Today, Planrooms, Runrooms), review views (Sweep, Analytics), and an admin section (Billing, Newsletter, Rooms, Agents, Mailroom, SoCal, System — the last a server-health readout that polls only while open). Built with React and Express, served on a single port, accessible from any machine on the Tailscale network. Re-clicking the active nav item resets that view to its landing state. See [`dashboard/README.md`](dashboard/README.md) for setup and usage details.

- **Board view:** Projects appear as draggable cards organized into status columns (`In Progress`, `WFHuman`, `Ice`, `Completed`). Cards show company, division, notes, tmux session, and next step at a glance. A blue badge shows the count of active inbox items linked to the project. Projects auto-created by the inbox pipeline show the OpenDia mark at 50% opacity in the top-right corner (brightens to 85% on hover; tooltip shows the originating email subject). A 60s server-side **status reconciler** owns the Ice ↔ In Progress boundary: running timer forces In Progress, `standing`-tagged cards ice unless timered, and dated next_steps bounce cards up within 3 days of their date and back to Ice beyond it (see the dashboard README's "Status reconciler" for the exact rules).
...
- **Wrong recommendation? Adjust it.** A typed correction outranks the recommendation, the scan's findings and the brief. Nothing is performed; a revised step comes back, sometimes routed differently than the one you rejected.
- **Certitude moves the emphasis.** Below 60 — where the rubric says the fronts disagreed or a decisive one could not be read — Adjust becomes the primary button and Do it is demoted, because a step Spark is not confident in is one to correct rather than point automation at.
- **A runroom is one click.** Open a runroom seeds `plan.json` from the step, writes a handoff brief, **closes the Spark timer**, spawns a named tmux session on it, and points the card there. The timer closes *before* the spawn on purpose: the runroom contract says room work bills to the session's own timer and never opens a second one, so spawning first would double-bill the card.
- **Rooms resolve through the card, and are disposable.** The runrooms list treats a room as *resolved* when its own plan finishes **or its card is completed** — derived at read time, `plan.json` stays the session's to write. Resolved rooms collapse behind a "Resolved (N)" toggle; each open row carries one-click **Open card** and a two-click **Complete card**. When there is nothing to do *right now*, **Park** writes a dated `next_step` onto the card and tells the session to wrap the room up: the card (which the reconciler ices until three days before the date, calendar event intact) is what hibernates and comes back — a fresh plan and a fresh room get born when it wakes, the old room never reopens. Close-out can still ask questions *after* a plan flips to finished (a timer-estimate or due-date dialog), so a finished room stays interactive — dialogs rendered and answerable — for as long as its tmux session is visibly wrapping up.
- **The routes are enforced, not requested.** `--disallowedTools` removes tools from the session outright (it holds even under bypassed permissions, unlike `--allowedTools`, which is merely additive), and a PreToolUse guard hook fences the shell, which has no tool name to deny — no outbound mail, no remote systems, no git writes, no file writes outside the run directory. Every refusal is logged. The model never holds the ability to send email.
- **It bills like a human would.** A Spark opens a timer at a 15-minute estimate — the cost of diving back into a project to refresh yourself — and each step carried out adds its own minutes, pausing at 60 so continuing becomes an explicit decision. The timer starts *lazily*, on the first completed front, so a run that dies in its first twenty seconds leaves nothing in the ledger. Its state file carries a deliberately empty session name so the Terminal tab's own timer logic can never confuse the two.
- **It can't strand a timer.** A Spark holds the card's timer while it waits on your decision, and its state file deliberately carries no session name — which means `/od-stop` in a terminal can never find it. So the pane says plainly what it is holding and counts down to when it lets go, "Discuss in Terminal" writes the brief and closes the timer on the way out, and a run interrupted by a restart is *recovered* at boot from its own report — rounds, accrual and all.
- **Every run is kept.** The brief, the raw stream and the report all persist per run, so any past spark can be reopened. The report file is rewritten after each round, so it records what the run concluded rather than what it first guessed.
